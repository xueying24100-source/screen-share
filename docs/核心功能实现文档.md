...
| `shareScreenRequested(source)` | 选择共享来源后点击开始共享 | 携带 `sourceId/sourceType` 请求开始共享 |
| `stopShareRequested()` | 点击停止共享 | 停止屏幕采集 |
| `grabShareRequested()` | 点击抢共享 | 切换共享者 |
| `micToggleRequested(bool)` | 点击麦克风 | 开关麦克风 |
| `annotationStrokeCommitted(points)` | 本地完成一笔画笔 | 同步归一化画笔坐标 |
| `annotationsCleared()` | 点击清空标注 | 同步清空画笔 |
...
graph TD
    ...
    C --> D[4. 选择共享来源并发送 sourceId/sourceType]
    D --> E[5. share_started 广播后启动本机采集]
    E --> F[6. 连接 frameCaptured 信号到 ScreenView 渲染]
    F --> G[7. 通过 annotation 消息同步画笔]
```

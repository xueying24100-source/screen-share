                             ...
                             const QString &serverHost, quint16 serverPort)
{
    ...
    connect(m_roomPage, &RoomPage::shareScreenRequested,
            this, [this](const ScreenCaptureSourceInfo &source) {
                m_client->requestShareStart(source.id, static_cast<int>(source.type));
            });
    ...
    connect(m_roomPage, &RoomPage::annotationStrokeCommitted,
            this, [this](const QVector<QPointF> &points) {
                m_client->sendAnnotationStroke(points);
            });
    connect(m_roomPage, &RoomPage::annotationsCleared,
            this, [this]() { m_client->sendAnnotationClear(); });
    connect(m_client, &RoomClient::annotationStrokeReceived,
            m_roomPage, &RoomPage::onRemoteAnnotationStroke);
    connect(m_client, &RoomClient::annotationClearReceived,
            m_roomPage, &RoomPage::onRemoteAnnotationClear);
    ...
    // 设置房间信息并切换页面
    RoomPageInfo info;
    info.clientId = m_client->clientId();

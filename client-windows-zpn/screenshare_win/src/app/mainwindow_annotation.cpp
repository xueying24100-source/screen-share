...
QPixmap MainWindow::captureWindowPixmap(quintptr windowHandle) const
{
    ...
    // 对非最小化窗口，先抓屏幕上的实时可见区域。这样窗口内容滚动、网页刷新、
    // PDF 翻页时不会只停留在 PrintWindow 返回的初始化帧。
    // 如果目标窗口被遮挡或裁剪结果异常，后面再退回 PrintWindow / BitBlt。
    if (!IsIconic(hwnd)) {
        QPixmap live = captureVisibleWindowPixmap(windowHandle);
        if (!pixmapLooksMostlyBlack(live)) {
            return live;
        }
    }

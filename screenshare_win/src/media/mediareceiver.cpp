...
void MediaReceiver::onPacketReceived(const QByteArray &packet)
{
    ...
    if (streamType == StreamType::Control) {
        ...
        if (sourceKind == SourceKind::DesktopMain && payload == QByteArray("share_off")) {
            const quint8 mainKind = static_cast<quint8>(SourceKind::DesktopMain);
            m_ignoreNextMainDecodedFrame = m_videoDecodeInFlight.value(mainKind, false);
            m_videoDecodeInFlight[mainKind] = false;
            m_pendingVideoPayload.remove(mainKind);
            emit mainVideoStoppedReceived();
            return;
        }
...
void MediaReceiver::onVideoFrameDecoded(quint8 sourceKindRaw, const QImage &image)
{
    ...
    if (sourceKind == SourceKind::CameraPip) {
        ...
        if (m_ignoreNextMainDecodedFrame) {
            m_ignoreNextMainDecodedFrame = false;
            submitPendingVideoIfAny(sourceKindRaw);
            return;
        }
...
void MediaReceiver::onVideoDecodeFailed(quint8 sourceKindRaw, const QString &message)
{
    ...
    if (sourceKindRaw == static_cast<quint8>(SourceKind::DesktopMain)) {
        m_ignoreNextMainDecodedFrame = false;
    }

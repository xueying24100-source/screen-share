...
void MemberList::addMember(const MemberInfo &member)
{
    ...
    item->setData(Qt::UserRole, member.clientId);
...
void MemberList::removeMember(const QString &clientId)
{
...
void MemberList::setMyClientId(const QString &clientId)
{
    m_myClientId = clientId;
}

void MemberList::updateMemberSharing(const QString &clientId, bool isSharing)
{
    for (int i = 0; i < count(); ++i) {
        QListWidgetItem *item = this->item(i);
        if (item->data(Qt::UserRole).toString() == clientId) {
            const QString name = item->data(Qt::UserRole + 1).toString();
            bool isMe = (clientId == m_myClientId);
            MemberInfo info{clientId, name, isSharing, isMe};
